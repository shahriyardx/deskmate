...
const OpenPRs = () => {
  const { data, isLoading } = trpc.github.getOpenPullRequests.useQuery()
      ...
      <Dialog>
        <DialogTrigger asChild>
          <p className="flex items-center gap-2 hover:underline cursor-pointer">
            <GitPullRequest size={18} /> {data.total_count} PRs waiting for your
            review
          </p>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Open PRs</DialogTitle>
            <DialogDescription>PRs waiting for your review</DialogDescription>
          </DialogHeader>
